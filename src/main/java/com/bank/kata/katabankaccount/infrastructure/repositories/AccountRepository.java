...
public interface AccountRepository extends JpaRepository<Account, Long> {
    @Query(value = """
            select new com.bank.kata.katabankaccount.core.domain.AccountStatementDTO(c.firstName, c.lastName,
            ac.type, ac.balance, t.amount, t.type, t.description)
            from Client c join c.accounts ac left join ac.transactions t
            where ac.id = :accountId
            """)
    List<AccountStatementDTO> fetchAccountStatement(@Param("accountId") Long accountId);
}
